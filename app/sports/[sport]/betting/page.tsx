...
export default async function SportBettingPage({ params }: { params: { sport: string } }) {
  ...
  // Fantasy sports get a mode chooser above this page; the rest come from home.
  const backHref  = hasFantasy(sport) ? `/sports/${sport}` : '/'
  const backLabel = hasFantasy(sport) ? `← ${SPORTS[sport].label}` : '← Sports'

  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3">
        <Link href={backHref} className="text-slate hover:text-white text-sm transition-colors">{backLabel}</Link>
        <span className="text-rail">/</span>
        <h1 className="text-2xl font-bold text-white">
          {SPORTS[sport].emoji} {SPORTS[sport].label} Betting
        </h1>
      </div>
